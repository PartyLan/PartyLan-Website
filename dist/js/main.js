...
function initContact(component){
  ...
  var intent=form.elements.intent, pkg=form.elements.package, subject=form.elements.subject, fields=[].slice.call(form.querySelectorAll('.field--event')), disclosure=form.querySelector('.contact-disclosure'), eventToggle=form.querySelector('[data-event-toggle]'), eventDetailsRevealed=false;
  ...
  intent.value=supportedIntent(qi||component.dataset.defaultIntent); pkg.value=packages.indexOf(qp)>=0?qp:(packages.indexOf(component.dataset.defaultPackage)>=0?component.dataset.defaultPackage:'unsure');
  ...
  function submitWeb3Form(targetForm){
    syncContactContext();
    return fetch(targetForm.action,{method:'POST',body:new FormData(targetForm),headers:{'Accept':'application/json'}}).then(function(r){return r.json().catch(function(){return{};}).then(function(j){if(!r.ok||j.success===false)throw {status:r.status,json:j}; return j;});});
  }
  form.addEventListener('submit',function(e){e.preventDefault(); status.className='contact-status is-loading'; status.textContent=''; if(!validate())return; var btn=form.querySelector('[type=submit]'); if(btn.disabled)return; var originalLabel=btn.textContent; btn.disabled=true; form.setAttribute('aria-busy','true'); btn.textContent='Sending enquiry…'; status.textContent='Sending enquiry…';
    submitWeb3Form(form).then(function(){form.reset(); intent.value=supportedIntent(component.dataset.defaultIntent); pkg.value=packages.indexOf(component.dataset.defaultPackage)>=0?component.dataset.defaultPackage:'unsure'; eventDetailsRevealed=intent.value==='booking'; syncContactContext(); status.className='contact-status is-success'; status.textContent='Thanks — your enquiry has been sent. We’ll get back to you as soon as possible.'; form.hidden=true; success.hidden=false; success.focus&&success.focus();}).catch(function(ex){console.error('Web3Forms contact submission failed',ex); status.className='contact-status is-error'; status.textContent='We couldn’t send your enquiry. Please try again, or contact Party.LAN directly by email.'; status.focus&&status.focus();}).finally(function(){btn.disabled=false; form.removeAttribute('aria-busy'); btn.textContent=originalLabel||btn.dataset.submitLabel||'Send enquiry';});
  });
  var again=component.querySelector('[data-send-another]'); if(again)again.addEventListener('click',function(){success.hidden=true; form.hidden=false; status.className='contact-status'; status.textContent=''; form.querySelectorAll('[aria-invalid="true"]').forEach(function(el){el.setAttribute('aria-invalid','false')}); form.querySelectorAll('[data-error-for]').forEach(function(el){el.textContent=''}); intent.value=supportedIntent(component.dataset.defaultIntent); pkg.value=packages.indexOf(component.dataset.defaultPackage)>=0?component.dataset.defaultPackage:'unsure'; eventDetailsRevealed=intent.value==='booking'; syncContactContext(); form.focus();});
  form.syncContactContext=syncContactContext;
  form.setContactIntent=function(nextIntent,nextPackage){intent.value=supportedIntent(nextIntent); if(packages.indexOf(nextPackage)>=0)pkg.value=nextPackage; eventDetailsRevealed=intent.value==='booking'; syncContactContext();};
}
