...
def validate_home(h):
    ...
    if h.get('header',{}).get('availability_cta',{}).get('href')!='/contact/?intent=availability': err(f,'header.availability_cta.href','expected /contact/?intent=availability')
...
def faq(home, rows):
    ...
    for r in rows:
        ...
        parts.append(f'<div class="faq-item"><h3><button aria-expanded="false" aria-controls="faq-{r["id"]}" id="faq-btn-{r["id"]}">{esc(r["question"])}<span aria-hidden="true">+</span></button></h3><div class="faq-item__answer" id="faq-{r["id"]}" role="region" aria-labelledby="faq-btn-{r["id"]}"><div class="faq-item__answer-inner"><p>{esc(r["answer"])}</p>{link}</div></div></div>')
    items=''.join(parts)
    f=home['faq_section']
...
def package_page(home, pkgs, addons, faq_rows):
...
def contact_page(home):
    return head(home,'Contact Party.LAN','Check availability, start planning your party or ask Party.LAN a question.')+f"""<body id=\"top\"><div class=\"site-background\" aria-hidden=\"true\"><div class=\"site-background__top\"></div><div class=\"site-background__middle\"></div><div class=\"site-background__bottom\"></div></div><a class=\"skip-link\" href=\"#main\">Skip to content</a>{header(home,'/')}<main id=\"main\" class=\"site-shell contact-page\"><section class=\"section contact-section\" aria-labelledby=\"contact-title\"><div class=\"section-heading reveal\"><p class=\"eyebrow\">CONTACT</p><h1 id=\"contact-title\">How can we help?</h1><p>Check availability, start planning your party or ask us anything before deciding.</p></div>{contact_form('contact-page-form')}</section></main>{footer(home,'/')}<script src=\"/js/main.js\" defer></script></body></html>"""
...
def main():
    ...
    (DIST/'packages').mkdir(); (DIST/'packages'/'index.html').write_text(package_page(home,pkgs,addons,faq_rows),encoding='utf-8')
    ...
    (DIST/'contact').mkdir(); (DIST/'contact'/'index.html').write_text(contact_page(home),encoding='utf-8')
