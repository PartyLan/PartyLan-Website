...
# Render the Header/Subtext pair consistently wherever text sits over media.
# Semantic headings preserve the document outline; paragraphs hold supporting copy.
def media_caption(header, subtext, tag='div', class_name=''):
    classes='media-caption'+((' '+class_name) if class_name else '')
    return f'<{tag} class="{classes}"><h3 class="media-caption__header">{esc(header)}</h3><p class="media-caption__subtext">{esc(subtext)}</p></{tag}>'
...
def showcase(home,gallery):
    ...
    slides=''.join(f'<figure class="showcase-slide {"is-active" if i==0 else ""}" data-category="{r["category"]}"><div class="showcase-slide__pan"><div class="showcase-slide__breathe"><img src="{r["image"]}" alt="{esc(r["Header"]+". "+r["Subtext"])}" loading="eager" decoding="async"></div></div>{media_caption(r["Header"],r["Subtext"],tag="figcaption")}</figure>' for i,r in enumerate(gallery))
