...
def faq(home, rows):
    ...
    for r in rows:
        ...
        parts.append(f'<div class="faq-item"><h3><button aria-expanded="false" aria-controls="faq-{r["id"]}" id="faq-btn-{r["id"]}"><span class="rollout-control__label">{esc(r["question"])}</span><span class="rollout-control__icon" data-rollout-icon aria-hidden="true">⌄</span></button></h3><div class="faq-item__answer" id="faq-{r["id"]}" role="region" aria-labelledby="faq-btn-{r["id"]}"><div class="faq-item__answer-inner"><p>{esc(r["answer"])}</p>{link}</div></div></div>')
    items=''.join(parts)
    f=home['faq_section']
