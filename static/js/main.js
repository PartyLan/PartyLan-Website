(function(){
  ...
  function apply(theme,save){document.documentElement.dataset.theme=theme;setImages(theme);if(toggle){var dark=theme==='dark';toggle.setAttribute('aria-pressed',String(dark));var label=dark?'Bright':'Dark';var text=toggle.querySelector('.theme-toggle__label');if(text){text.textContent=label;}else{toggle.textContent=label;}toggle.setAttribute('aria-label',dark?'Switch to bright theme':'Switch to dark theme');toggle.setAttribute('title',dark?'Switch to bright theme':'Switch to dark theme');}if(save){try{localStorage.setItem(storageKey,theme);}catch(e){}}}
  ...
  document.querySelectorAll('.testimonials').forEach(function(section){var slides=[].slice.call(section.querySelectorAll('.testimonial-slide'));var dots=[].slice.call(section.querySelectorAll('.testimonial-dot'));if(slides.length<2||!dots.length){return;}function show(index){slides.forEach(function(slide,i){var active=i===index;slide.classList.toggle('is-active',active);slide.setAttribute('aria-hidden',String(!active));});dots.forEach(function(dot,i){var active=i===index;dot.classList.toggle('is-active',active);dot.setAttribute('aria-current',active?'true':'false');});}section.classList.add('is-enhanced');dots.forEach(function(dot,index){dot.addEventListener('click',function(){show(index);});});show(0);});
